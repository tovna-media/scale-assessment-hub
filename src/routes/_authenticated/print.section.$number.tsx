...
function PrintSectionPage() {
  ...
  useEffect(() => {
    if (!user) return;
    (async () => {
      const [sub, prof, row] = await Promise.all([
      ...
      setData((row.data?.data as Record<string, unknown>) ?? null);
      setLoading(false);
    })();
  }, [user, sectionNumber, checkSub]);
    ...
    <>
      <div className="no-print sticky top-0 z-10 flex items-center justify-between border-b border-border bg-white/90 px-4 py-3 backdrop-blur">
        ...
        <p className="text-xs text-muted-foreground">
          {SECTION_TITLES[sectionNumber] || `Section ${sectionNumber}`}
        </p>
      ...
      <main className="print-doc mx-auto max-w-3xl bg-white px-8 py-10 text-black">
        ...
        <SectionPrint sectionNumber={sectionNumber} data={data} />
        <PrintFooter />
      </main>
    </>
  );
